...
/// Native replica of the circuit's K_MAC derivation. Returns the 32-byte block
/// the circuit reads from `p_outputs[0]` (the first key-expansion P-hash block).
/// The circuit binds the first 248 bits of this to `p_fe + v_fe`, so the
/// coordinator must build its shares from THIS value for the proof to satisfy.
pub fn circuit_k_mac_native(
    pms_bytes:     &[u8; 32],
    client_random: &[u8; 32],
    server_random: &[u8; 32],
) -> [u8; 32] {
    // Phase 2: master secret = TLS-PRF(PMS, "master secret", cr ‖ sr)[..32]
    let mut cr_sr = client_random.to_vec();
    cr_sr.extend_from_slice(server_random);
    let ms_vec = tls_prf_native(pms_bytes, LABEL_MASTER_SECRET, &cr_sr, 32);
    let mut ms_key = [0u8; 32];
    ms_key.copy_from_slice(&ms_vec);

    // key expansion seed = "key expansion" ‖ sr ‖ cr  (matches circuit order)
    let mut seed_ke = LABEL_KEY_EXPANSION.to_vec();
    seed_ke.extend_from_slice(server_random);
    seed_ke.extend_from_slice(client_random);

    // First P-hash block: A1 = HMAC(ms, seed); P1 = HMAC(ms, A1 ‖ seed)
    let a1 = hmac_sha256_native(&ms_key, &seed_ke);
    let mut p_input = a1.clone();
    p_input.extend_from_slice(&seed_ke);
    let p1 = hmac_sha256_native(&ms_key, &p_input);
    let mut out = [0u8; 32];
    out.copy_from_slice(&p1);
    out
}

/// 248-bit field encoding of a 32-byte value (first 31 bytes, LE), matching the
/// circuit's `k_mac_fe_248`. Exposed so the coordinator can build consistent
/// shares and commitment.
pub fn k_mac_fe_248_pub<F: PrimeField>(bytes: &[u8; 32]) -> F {
    k_mac_fe_248::<F>(bytes)
}
...
pub struct TlsPrfCircuit<F: PrimeField> {
    ...
    pub commitment:    F,
    pub rand_binding:  F,
    /// MPC mode: pre-computed shared field element for this party's K_MAC share P.
    /// `None` in central mode (then `p_share` bytes are used).
    pub p_share_fe:    Option<F>,
    /// MPC mode: pre-computed shared field element for this party's K_MAC share V.
...
impl<F: PrimeField> TlsPrfCircuit<F> {
    ...
    /// MPC constructor. Each party supplies its **shared** K_MAC share field
    /// elements (`p_fe`, `v_fe`) and the public `commitment` / `rand_binding`.
    /// The PMS and TLS randoms are the cleartext inputs the PRF is computed
    /// over; under MPC the SHA-256 wires they produce are public scalars, which
    /// the collaborative-zksnark MSM now promotes to trivially-shared values,
    /// so the full SHA-256 TLS-PRF runs inside the proof. Only the final K_MAC
    /// binding (p_fe + v_fe) is genuinely secret-shared.
...
impl<F: PrimeField> ConstraintSynthesizer<F> for TlsPrfCircuit<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        ...
        //   MPC mode: use the pre-computed shared field elements directly so the
        //   K_MAC binding is genuinely secret-shared. Central mode: derive from bytes.
        let p_fe  = self.p_share_fe.unwrap_or_else(|| k_mac_fe_248::<F>(&self.p_share));
        let v_fe  = self.v_share_fe.unwrap_or_else(|| k_mac_fe_248::<F>(&self.v_share));
