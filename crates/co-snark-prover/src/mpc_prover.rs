...
pub fn prove_distributed(req: ProverRequest) -> ProverResponse {
    ...
    // In full-circuit (TLS-PRF) mode the circuit enforces
    //   PRF(PMS, randoms) == fe248(p) + fe248(v)
    // so the shares cannot be arbitrary — they must add up to the PRF output.
    // We compute the circuit's K_MAC block natively and set the shares to
    //   p_bytes = K_MAC_block,  v_bytes = 0
    // (additive secret-sharing inside MPC still splits them via king_share).
    // In key-circuit mode the original p, v, XOR semantics are kept.
    let (p, v, k_mac) = if use_full_circuit() {
        // Synthetic PRF inputs — must match the pms/cr/sr the parties receive
        // in run_two_parties (kept in sync below).
        let pms = [0u8; 32];
        let cr  = [0xAAu8; 32];
        let sr  = [0xBBu8; 32];
        let k_mac_block = crate::tls_prf_circuit::circuit_k_mac_native(&pms, &cr, &sr);
        // p carries the full K_MAC block, v is zero → fe248(p)+fe248(v)=fe248(K_MAC).
        (k_mac_block, [0u8; 32], k_mac_block)
    } else {
        (p, v, xor_shares(&p, &v))
    };
    ...
    let params: ProvingKey<Bls12_377> = if !req.crs_file.is_empty() {
        ...
        // Distributed MPC: pick the CRS circuit to match the proving circuit.
        // full_circuit → TlsPrfCircuit (full SHA-256 TLS-PRF); else XOR-only.
        if use_full_circuit() {
            eprintln!("[co-snark-prover] distributed: no CRS — running setup (TlsPrfCircuit)");
            match generate_random_parameters::<Bls12_377, _, _>(
                crate::tls_prf_circuit::TlsPrfCircuit::<Fr>::dummy(), &mut OsRng)
            {
                Ok(pk) => pk,
...
fn run_as_party(v: &serde_json::Value) -> ProverResponse {
    ...
    // Encoding must match the active circuit (248-bit for TlsPrfCircuit).
    let my_share_fe: Fr = if full_circ {
        crate::tls_prf_circuit::k_mac_fe_248_pub::<Fr>(&my_share_bytes)
    } else {
        bytes32_to_field::<Fr>(&my_share_bytes)
    };
    ...
    // Optional WAN latency simulation via MPC_LATENCY_MS env var (one-way RTT).
    // The 2-party MPC Groth16 has ~3 sequential communication rounds (input
    // sharing, multiplication openings, output reconstruction), so the total
    // network cost is ~3×RTT. Applied after mpc_create_proof to avoid tripping
    // the MPC library's TCP connection timeout on high-latency profiles.
    const MPC_ROUNDS: u64 = 3;
    let mpc_rtt_ms: u64 = std::env::var("MPC_LATENCY_MS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(0);
    let mpc_net_ms = mpc_rtt_ms.saturating_mul(MPC_ROUNDS);
    ...
    // Distributed MPC now supports the FULL TLS-PRF circuit. The
    // collaborative-zksnark MSM promotes public scalars (SHA-256 round
    // constants / cleartext-derived wires) to trivially-shared values, so the
    // homogeneity requirement is satisfied and the SHA-256 TLS-PRF runs inside
    // the 2-party proof. Only the K_MAC binding (p_fe + v_fe) is secret-shared.
    // When `full_circuit` is false we keep the fast XOR-only TlsKeyCircuit.
    let mpc_proof = {
        if full_circ {
            // Read the PRF cleartext inputs (PMS + TLS randoms) from the request.
            let pms_b = v["pms_hex"].as_str()
                .and_then(|s| decode32(s).ok()).unwrap_or([0u8; 32]);
            let cr_b = v["cr_hex"].as_str()
                .and_then(|s| decode32(s).ok()).unwrap_or([0xAAu8; 32]);
            let sr_b = v["sr_hex"].as_str()
                .and_then(|s| decode32(s).ok()).unwrap_or([0xBBu8; 32]);
            let circuit = crate::tls_prf_circuit::TlsPrfCircuit::<MpcFr>::new_mpc(
                pms_b, cr_b, sr_b,
            ...
            if mpc_net_ms > 0 {
                std::thread::sleep(std::time::Duration::from_millis(mpc_net_ms));
            }
            result
        } else {
...
fn compute_commitment(p: &[u8; 32], v: &[u8; 32], rb: &[u8; 32]) -> Vec<u8> {
    ...
    // commitment = fe(p_share) + fe(v_share) + rand
    // Encoding MUST match the active circuit:
    //   full_circuit → k_mac_fe_248 (31-byte/248-bit, matches TlsPrfCircuit)
    //   key_circuit  → bytes32_to_field (full 32-byte, matches TlsKeyCircuit)
    let (p_fe, v_fe, rb_fe) = if use_full_circuit() {
        (
            crate::tls_prf_circuit::k_mac_fe_248_pub::<Fr>(p),
            crate::tls_prf_circuit::k_mac_fe_248_pub::<Fr>(v),
            bytes32_to_field::<Fr>(rb),
        )
    } else {
        (
            bytes32_to_field::<Fr>(p),
            bytes32_to_field::<Fr>(v),
            bytes32_to_field::<Fr>(rb),
        )
    };
