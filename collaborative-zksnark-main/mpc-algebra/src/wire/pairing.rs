        ...
        impl<E: PairingEngine, PS: PairingShare<E>> AffineCurve for $w_aff<E, PS> {
            ...
            fn multi_scalar_mul(bases: &[Self], scalars: &[Self::ScalarField]) -> Self::Projective {
                let b = {
                    ...
                    // Mixed public/shared scalars are allowed: promote public scalars to
                    // trivially-shared values so the shared MSM path handles everything.
                    // This is needed when boolean gadgets (e.g. SHA-256 round constants)
                    // produce public scalars alongside shared witness scalars.
                    let scalars_normalized: Vec<Self::ScalarField> = scalars.iter().map(|s| {
                        match s {
                            MpcField::Public(x) => MpcField::Shared(
                                <PS::FrShare as Reveal>::from_public(*x)
                            ),
                            MpcField::Shared(_) => s.clone(),
                        }
                    }).collect();
                    let scalars = &scalars_normalized;
                    let bases =
